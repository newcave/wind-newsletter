...
반드시 마크다운·코드블록 없이 순수 JSON 배열만 반환:
[
  {{
    "category": "카테고리코드",
    "title": "뉴스 제목 (한국어)",
    ...
    # output_text 추출 (여러 방식 순서대로 시도)
    text = ""
    # 방법1: output_text 직접 속성
...
def render_sidebar(secrets_key):
    with st.sidebar:
        ...
        if secrets_key:
            st.success("🔐 OpenAI API Key: Secrets 적용됨")
            api_key = secrets_key
        else:
            ...
            with st.spinner("🌬 AI가 최신 풍력 뉴스를 검색·요약 중입니다..."):
                try:
                    ...
                    if "401" in msg or "authentication" in msg.lower() or "api_key" in msg.lower():
                        st.error("❌ API Key가 유효하지 않습니다. OpenAI API Key를 확인하세요.")
                    else:
                        st.error(f"❌ 오류: {ex}")
